...
from utils.config import Config, PytorchBatchConfig, LifeLongConfig, WandBConfig, VARS, SOMConfig, TopologicalAEConfig
...
if __name__ == '__main__':
	...
	for (var1, var2) in param_combinations:
		...
		config = Config(
			...
			LifeLong_config=LifeLongConfig(SIGMA=10, ALPHA=None, BETA=0.02, BATCH_SIZE=2, EPOCHS_PER_SUBSET=2, SUBSET_SIZE=1, DISJOINT_TRAINING=True, LR_GLOBAL_BASELINE=0.1, SIGMA_BASELINE=1.5, LEARNING_RATE=0.1, TARGET_RADIUS=1.5, MODE="Base"),
			pytorch_batch_config=PytorchBatchConfig(EPOCHS=2, BATCH_SIZE=20, TARGET_RADIUS=1.5, LEARNING_RATE=0.001, BETA=0.01, SIGMA=10, MODE="Base"),
			variables=VARS()
		)
		runner.run(config)
