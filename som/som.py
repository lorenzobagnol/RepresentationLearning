...
class SOM(nn.Module):
	...
	"""
	def __init__(self, m: int , n: int, input_data: InputData, niter, alpha=None, sigma=None):
	...
	def forward(self, x: torch.Tensor, it: int):
		...
		learning_rate_multiplier = torch.stack([learning_rate_op[i:i+1]*np.ones(self.dim) for i in range(self.m*self.n)])
