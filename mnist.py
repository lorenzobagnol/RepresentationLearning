...
def run_experiment(*args, input_data, dataset_train, dataset_val):
	...
	try:
		...
		config = Config(
			...
			lifelong_config=LifeLongConfig(ALPHA=None, BETA=0.02, BATCH_SIZE=20, EPOCHS_PER_SUBSET=200, SUBSET_SIZE=1, DISJOINT_TRAINING=True, LR_GLOBAL_BASELINE=0.1, SIGMA_BASELINE=1.5, LEARNING_RATE=0.1, MODE="Base"),
			...
			variables=VARS(SEED=args[0], target_radius=1.5)
		)
