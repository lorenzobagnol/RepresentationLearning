...
def create_image_grid(alphas, betas, mode, sigma_baseline):
    
    download_dir=".\\downaloads"

    # Create a 3x3 grid of subplots for weight images
    fig, axes = plt.subplots(len(alphas), len(betas), figsize=(12, 12), sharex=True, sharey=True)
    fig.suptitle("Weight Images for sigma_baseline ="+str(sigma_baseline)+" ("+str(mode) +")")
...
def __main__():
    ...
    create_image_grid(alphas, betas, mode, sigma_baseline)
